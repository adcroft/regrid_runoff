...
def regrid_runoff( old_file, var_name, A, new_file_name, ocn_area, ocn_mask, ocn_qlat, ocn_qlon, ocn_lat, ocn_lon, rvr_area, fms_attr, num_records , toler=1e-15):
  ...
  new_file = netCDF4.Dataset(new_file_name, 'w', clobber=True, format="NETCDF4")
  ...
  area = new_file.createVariable('area', 'f4', ('j','i',))
  ...
  if '_FillValue' in old_file.variables[var_name].ncattrs():
    new_runoff = new_file.createVariable(var_name, 'f4', dims, fill_value = old_file.variables[var_name]._FillValue,zlib=True, complevel=1)
    if fms_attr: new_runoff.missing_value = old_file.variables[var_name]._FillValue
  else:
    new_runoff = new_file.createVariable(var_name, 'f4', dims,zlib=True, complevel=1)
